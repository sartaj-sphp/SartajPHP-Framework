...
(function () {
    ...
    const open = editor => {
      ...
      const segments = pathname.split('/').filter(segment => segment !== ''); // Split by '/' and remove empty strings
      const lastSegment = segments[segments.length - 1]; // Get the last elemen
      const dataApi = editor.windowManager.open({
        title: 'Preview:- ' + lastSegment,
        size: 'large',
        body: {
          type: 'panel',
          items: [{
    ...
    var Plugin = () => {
      global$2.add('plgpreview', editor => {
        register$1(editor);
        register(editor);
      });
    };
